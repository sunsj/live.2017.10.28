...
char* RTSPServer::rtspURLPrefix(int clientSocket) const
{
	...
	if (clientSocket < 0) 
	{
		...
		ourAddress.sin_addr.s_addr = ReceivingInterfaceAddr != 0 ? ReceivingInterfaceAddr : ourIPAddress(envir()); // hack
	} 
	else
	{
...
Boolean RTSPServer::setUpTunnelingOverHTTP(Port httpPort)
{
	...
	if (fHTTPServerSocket >= 0) 
	{
		...
		envir().taskScheduler().turnOnBackgroundReadHandling(fHTTPServerSocket, incomingConnectionHandlerHTTP, this);
		return True;
	}
...
void RTSPServer::RTSPClientSession
::handleCmd_TEARDOWN(RTSPServer::RTSPClientConnection* ourClientConnection, ServerMediaSubsession* subsession) 
{
	unsigned i;
